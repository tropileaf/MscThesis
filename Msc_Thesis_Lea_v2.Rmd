...
+ how to group all files (ex: all EBSAs + IBAs + IMMAs + vents + seamounts...) into one, to create ConsFeatures?
+ should we use Sharma (2011) + CRU Consulting (2019) to assign USD values to PMN areas? see PMN subsection
+ shipping raster (Halpern et al., 2013) is too heavy to be converted to polygon, what coupd be other methods (not using as.polygon)?
+ shipping raster : cells have certain values (# of ships recorded??I will check for Tuesday), could allow to infer USD value of route (# of ships * approx cost of fuel for average cargo to go through 1 PU, which is 1000sqkm)? So we wouldn't try to compute the cost of deviating from any given route, but rather compute the price for each ship to go through 1 PU * the number of ships going through that PU. It sounds fairly logical to me, but maybe it's not?
+ fishing cost layer : there is a ready-made fx in SpatPlan repo (thanks Jase & Tin), should we use it for this project? see fishing subsection (but issues with plotting  as you can see)
...
```{r, cache = FALSE, echo = FALSE, results = "hide", message = FALSE}
...
+ **select specific fishing rasters (bycatch/demersal...)/ use the fishing cost layer  already created in Spatial Planning repo**
+ and **select specific benthic shps (seamounts, ridges, plateaus, vents?)**
+ **need to create subset without inactive vents** *filter fx not working because object=character, will ask on Slack*
...
:-----------:|:----------------------:|:----------------------------------:
 ...
 COST        |  Shipping              |  Halpern et al., 2013 (tif)
 COST        |  Fishing               |  Halpern et al., 2013 (tif)
 ...
 CONS        |  Vents                 |  Beaulieu & Szanfranski (2020) (csv)
...
+ EBSAs
+ Deepsea features: vents, seamounts, ridges, plateaus? *(need to define)*

### Important Bird Areas

Conservation targets to be defined *need to write chunck to get ConsFeatures$amount*
...
Targets: *need to write chunck to get ConsFeatures$amount*
...
**Still unable to add legend in plots so showing the code instead, but will ask on Slack**

Seamounts, ridges, plateaus, vents

+ *need to add chunck for ridges & plateaus if we decide to add them*

+ *need to check how to 'filter' out inactive vents and to cross vents (point data) with PUs and how to add in plot*
...
PUxVents <- fSpatPlan_Get_Polyg(VentsActive, PUs)

```


```{r Conservation features, echo = FALSE, message = FALSE, warning = FALSE}
world <- ne_countries(scale = "medium", returnclass = "sf")
...
```{r Conservation features plot, echo = TRUE, message = FALSE, warning = FALSE}

ggplot() + 
  geom_sf(data = world) +
  geom_sf(data = IBA, size = 1, color = "green", fill = "green") + 
  geom_sf(data = IMMA, size = 1, color = "blue", fill = "blue")+ 
  geom_sf(data = NEIO_09, size = 1, color = "grey", fill = "grey")+ 
  geom_sf(data = NWIO_14, size = 1, color = "grey", fill = "grey")+
  geom_sf(data = SIO_11, size = 1, color = "grey", fill = "grey")+
  geom_sf(data = SIO_19, size = 1, color = "grey", fill = "grey")+
  geom_sf(data = SIO_22, size = 1, color = "grey", fill = "grey")+
  geom_sf(data = SIO_23, size = 1, color = "grey", fill = "grey")+
  geom_sf(data = SIO_30, size = 1, color = "grey", fill = "grey")+
  geom_sf(data = SIO_32, size = 1, color = "grey", fill = "grey")+
  geom_sf(data = SIO_35, size = 1, color = "grey", fill = "grey")+
  geom_sf(data = SIO_36, size = 1, color = "grey", fill = "grey")+
  geom_sf(data = SIO_37, size = 1, color = "grey", fill = "grey")+
  geom_sf(data = Seamounts, size = 1, color = "black", fill = "black")+
  ggtitle("IBA, IMMA, EBSAs, Seamounts") + 
...
+ Exploration & reserved areas of deep-sea mining (BUT these should be locked out rather than given a cost value)
+ Fishing effort (monetary value/index? using ready-made cost layer in Spatial Planning repo?)
+ Shipping routes (lock out? or use value of cell in Halpern et al., 2013 dataset?)
...
*Update* possibly able to have USD value for fishing & PMN (see PMN subset)
...
PUs <- PUs %>% 
  mutate(CFC = PUxCFC) # Add logical vector to variable called Mine1)Polyg1
```
...
*Still unable to add legend so showing the code, will ask on Slack*
...
```{r Mining layer plot, echo = TRUE, message = FALSE, warning = FALSE}
ggplot() + 
  geom_sf(data = world) +
  geom_sf(data = CFC, size = 1, color = "orange", fill = "orange") + 
  geom_sf(data = PMN, size = 1, color = "brown", fill = "brown") + 
  geom_sf(data = Expl_PMN, size = 1, color = "red", fill = "red") + 
  geom_sf(data = Expl_PMS, size = 1, color = "red", fill = "red") +
  geom_sf(data = Expl_CFC, size = 1, color = "red", fill = "red") +
  geom_sf(data = Res_PMN, size = 1, color = "red", fill = "red") +
  geom_sf(data = Res_CFC, size = 1, color = "red", fill = "red") +
...
**! cannot convert to polygon data when using code below (not written as code to avoid issues when knitting doc), error message says file too heavy** *will ask on Slack*
...
Fishing <- fSpatPlan_Get_Cost(PUs, cCRS) ## if issues, try fx below

FishingCost <- terra::rast("~/Documents/MscThesis/GitHub/SpatialPlanning/Data/Cost/Cost_Raster_Sum.grd") %>% 
    terra::as.polygons(trunc = FALSE, dissolve = FALSE, na.rm=FALSE) %>% # Convert to polygon data
    st_as_sf() %>% # Convert to sf
...
```
*Issues with plotting, should be a raster with colours based on cell USD value?*

```{r Fishing plot, echo = TRUE, message = FALSE, warnings = FALSE}
FishCost <- terra::rast("~/Documents/MscThesis/GitHub/SpatialPlanning/Data/Cost/Cost_Raster_Sum.grd")

plot(FishCost, main = 'Fishing cost')
```
...
```
p_ConsFeat <- problem(PUs, features = ConsFeatures, cost_column = "NoCost") %>% 
...
```
p_ConsFeat_Fishing <- problem(PUs, features = ConsFeatures, cost_column = "Fishing") %>% 
  add_min_set_objective() %>% 
  add_relative_targets(ConsFeatures$amount) %>% # representation targets (Area)
  add_binary_decisions() %>%
  add_lpsymphony_solver(verbose = FALSE) #%>%
