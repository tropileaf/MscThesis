...
+ how to assign price to mining shps / create cost layer?
+ assigning targets - Cons_feats tibble created
...
```{r Conservation features and targets, echo = FALSE, message = FALSE, warning = FALSE}
...
PUs <- PUs %>% 
  mutate(Fishing = FishingCost) #issues: no error but cannot check PU dataframe after this: Error in `[.data.frame`(x, i) : undefined columns selected; probably need to restart session
hist(FishingCost$FishingCost) 
#FishingCostPlusOne <- FishingCost$FishingCost + 1 #not needed, min >0 for PU=ABNJ
hist(log10(FishingCost$FishingCost)) #drop after log10(x)=4, x=10^4=10,000? #no longer needed since in ABNJ max val = ##USD 5000, and most values are 10^1 - 10^2 USD
...
#section not needed anymore since no outliers when PUs=ABNJ
...
p_ConsFeats <- problem(PUs, features = Cons_feats$Features) %>% #issues because no cost_column
...
p_ConsFeat_Fishing <- problem(PUs, features = Cons_feats$Features, cost_column = PUs$FishingCost) %>% 
  add_min_set_objective() %>% 
  add_relative_targets(Cons_feats$Targets) %>% 
  add_binary_decisions() %>%
  add_lpsymphony_solver(verbose = FALSE) 
...
p_ConsFeat_Mining <- problem(PUs, features = Cons_feats$Features, cost_column = PUs$PMNCost) %>% 
  add_min_set_objective() %>% 
  add_relative_targets(Cons_feats$Targets) %>% 
  add_binary_decisions() %>%
  add_lpsymphony_solver(verbose = FALSE) %>%
...
p_ConsFeat_MiningFishing <- problem(PUs, features = Cons_feats, cost_column = PUs$MiningFishing) %>% 
  add_min_set_objective() %>% 
  add_relative_targets(Cons_features$Targets) %>%
  add_binary_decisions() %>%
  add_lpsymphony_solver(verbose = FALSE) %>%
  add_locked_out_constraints("Expl_CFC", "Expl_PMN", "Expl_PMS", "Res_PMN", "Res_CFC")
```
...
*here, Shipping = mean values of pixels overlaid on PUs, so intensity of shippings; not USD* 

```{r Shipping problem, echo = TRUE, warning = FALSE}

p_ConsFeat_Shipping <- problem(PUs, features = Cons_feats$Features, cost_column = "PUs$Shipping") %>% 
  add_min_set_objective() %>% 
  add_relative_targets(Cons_feats$Targets) %>% 
  add_binary_decisions() %>%
  add_lpsymphony_solver(verbose = FALSE) #%>%
