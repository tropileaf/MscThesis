...
+ shipping raster (Halpern et al., 2013) is too heavy to be converted to polygon, what could be other methods (not using as.polygon)? RESAMPLE if resolution to fine
...
+ I'm using echo = FALSE, message = FALSE, warning = FALSE, results = "hide" and still can't get rid of some coding output in the .html :( 
+ I st_to_sf the vents from .csv with PUs' crs but have some issues intersecting the points with polygs, will look more into it. ASK ON SLACK
...
+ **using the fishing cost layer  already created in Spatial Planning repo**
+ deep-sea features = seamounts, plateaus, vents

still need to intersect vent points with PUs
...
+ EBSAs
+ Deepsea features: active vents, seamounts, rift_valleys, plateaus? *(need to define)*
...
Conservation targets to be defined *need to write chunck*
...
Seamounts, plateaus, active vents
Need to find out how to cross vents with PUs

Targets: tbd

Vents: >90% [.csv to .shp](https://datacarpentry.org/r-raster-vector-geospatial/10-vector-csv-to-shapefile-in-r/) 

Seamounts: >70% (ex: Letessier et al., 2019, PLOS Biology)
...
VentsActiveSF<- st_as_sf(VentsActive, coords = c("Latitude", "Longitude"), crs = st_crs(PUs)) # could rewrite section using pipes
```

```
PUxVents <- st_intersects(PUs, VentsActiveSF) %>% 
    as.logical() 
PUs <- PUs %>% 
  mutate(Vents = PUxVents) ##issue: NAs everywhere, probably because trying to cross points with polygs?
```
...
ggplot() + 
  ...
  scale_color_manual(values = c("A" = "grey", "B" = "black", "C" = "purple", "D" = "green", "E" = "blue", "F" = "red"), 
                     labels = c("EBSAs", "Seamounts", "Plateaus", "IBAs", "IMMAs", "Active vents"),
                     name = "Legend") 
...
+ Shipping routes (use value of cell in Halpern et al., 2013 dataset for relative cost)
...
ggplot() + 
  ...
  geom_sf(data = Expl_PMS, aes(color="C"), show.legend = "line") +
  geom_sf(data = Expl_CFC, aes(color="C"), show.legend = "line") +
  geom_sf(data = Res_PMN, aes(color="D"), show.legend = "line") +
  geom_sf(data = Res_CFC, aes(color="D"), show.legend = "line") +
  scale_color_manual(values = c("A" = "orange", "B" = "brown", "C" = "red", "D" = "red"), 
                     labels = c("CFC", "PMN", "Expl_PMN", "Expl_CFC", "Res_PMN", "Res_CFC"),
                     name = "Legend") +
...
**! cannot convert to polygon data when using code below (not written as code to avoid issues when knitting doc), error message says file too heavy** *Need to resample, will ask on Slack*
...
Could set cut-off for cost outliers at 15,000 USD or 40,000 USD - depending on whether we want to favour variation or follow standard procedures for cut-off (see histograms and maps).

```{r Fishing cost plot, echo = TRUE, message = FALSE, warnings = FALSE}

FishingCost <- fSpatPlan_Get_Cost <- function(PUs, cCRS){
  
  # TODO Add some filtering ability to get the cost only for specific groups
  
  Cost <- terra::rast("Data/Cost/Cost_Raster_Sum.grd") %>% 
    terra::as.polygons(trunc = FALSE, dissolve = FALSE, na.rm=FALSE) %>% # Convert to polygon data
    st_as_sf() %>% # Convert to sf
...
hist(FishingCost$Cost) #max value >200,000

ggplot() + 
  geom_sf(data = world) +
  geom_sf(data = FishingCost, aes(color=Cost)) +
ggtitle("Fishing Cost") + 
  coord_sf(xlim = c(IndianO[["xmin"]], IndianO[["xmax"]]), ylim = c(IndianO[["ymin"]], IndianO[["ymax"]]), expand = FALSE)

FishingCostOutliers <- filter(FishingCost, Cost >= 50000) # check out outliers, play with cut-off value
...
FishingCostNoOutliers <- filter(FishingCost, Cost <= 50000) # remove outliers, play with cut-off value

hist(FishingCostNoOutliers$Cost) # 49256 obs. = 524 outliers removed when cut-off = 20,000) but would also make sense to set cut-off at 15,000 or 40,000...
